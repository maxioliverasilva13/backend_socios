...
const getDepartamento = async (req = request, res = response) => {
  try {
    ...
    if (id) {
      const { localidad } = await getRepository(User).findOne({
        id: id,
        relations: ["localidad"],
      });
      const dptoId = await getRepository(Localidad).findOne({
        id: localidad.id,
        relations: ["departamento"],
      });
      if (dptoId) {
        const departamentoUser = await getRepository(Departamento).findOne({
          id: dptoId.id,
        });
        return res.json({
          ok: true,
...
const deleteDepartamento = async (req = request, res = response) => {
  try {
    const departamento = await getRepository(Departamento).delete(
      req.params.id
    );
    res.json({
      ok: true,
