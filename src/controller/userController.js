...
const getTokenWithG = async (req = request, res = response) => {
  try {
    ...
    const empresaWork = await getRepository(Empleado).find({
      where: { user: user?.id },
      relations: ["empresa"],
    });
...
const validarTokenUser = async (req = request, res = response) => {
  try {
    ...
    if (id) {
      ...
      if (usuario?.rol?.id == 3) {
        const empleado = await getRepository(Empleado).findOne({
          relations: ["user", "empresa", "cargo"],
          where: { user: usuario.id },
        });
        if (empleado?.empresa?.id) {
          const emp = await getRepository(Empresa).findOne({
            where: { id: empleado.empresa.id },
            relations: ["localidad"],
          });
          const rubros = await getRepository(EmpresaRubroA).find({
